...
export default function TractorShareApp() {
  ...
  useEffect(() => {
    ...
    const unsubBookings = onSnapshot(query(bookingsRef), (snap) => {
  ...
  const handleAddListing = async (e) => {
      ...
      const data = {
          ...
          image: "https://images.unsplash.com/photo-1515676664795-a73542c9016b?auto=format&fit=crop&q=80&w=800",
          rating: 5.0,
          verified: true,
  ...
  const handleSeedData = async () => {
    if (!user) {
        alert("Wait! You are not logged in yet.");
        return;
    }
    const listingsRef = collection(db, 'artifacts', appId, 'public', 'data', 'listings');
    try {
        ...
        alert("Success! Marketplace seeded.");
        setActiveTab('home'); 
    } catch (err) {
  ...
  // --- VIEW RENDERERS (These were likely missing!) ---
  ...
  const renderProfile = () => (
      <div className="pb-24 p-4">
          ...
          <div className="space-y-2">
              ...
              <button onClick={handleSignOut} className="w-full bg-white p-4 rounded-xl shadow-sm flex items-center justify-between hover:bg-gray-50 text-red-500">
                  <div className="flex items-center gap-3">
                      <LogOut size={20} />
